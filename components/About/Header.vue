<template>
    <header class="header-section" style="background: linear-gradient(90deg, #0000E4 0%, #0272DA 35%, #0055E7 100%);">
        <div class="overlay d-flex align-items-center h-100">
            <section class="w-100">
                <div class="container text-center text-white">
                    <div class="row">
                        <div class="col" data-aos="fade-in">
                            <h1>About Brieflly</h1>
                            <p class="lead">Brieflly started with a simple goal: to take the frustration out of finding
                                and working with service providers.</p>
                            <p class="lead">After years of dealing with mismatched briefs and under-delivered projects,
                                founders Paul and Joann knew there had to be a better way.</p>
                            <p class="lead">That's why Brieflly was created—to eliminate the chaos. Now, businesses can
                                generate automated project briefs in minutes, instantly connect with vetted, top-tier
                                agencies, and manage their projects from start to finish.</p>
                        </div>
                    </div>
                </div>
            </section>
        </div>
    </header>
</template>